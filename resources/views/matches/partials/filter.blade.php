<section class="jumbotron">
    <div class="container">
        ...
        <form method="GET" action="{{ route('matches.index') }}">
            ...
            @foreach(['action', 'card', 'collection', 'collector', 'currency', 'format', 'sort'] as $filter)
                @if($request->has($filter) && $request->filled($filter))
                    <a href="{{ route('matches.index', $request->except($filter, 'page')) }}" style="text-decoration: none;" class="badge badge-light text-uppercase mr-2">
                        <i class="fa fa-times text-danger"></i>
                        @if(in_array($filter, ['action', 'card', 'collection', 'currency', 'format']))
                            {{ $request->{$filter} }}
                        @else
                            {{ ucfirst($filter) }}
                        @endif
