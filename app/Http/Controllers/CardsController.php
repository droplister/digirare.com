...
class CardsController extends Controller
{
    ...
    public function show(Request $request, Card $card)
    {
        ...
        $buy_orders = $token ? $token->getOrders()->where('status', '=', 'open')->where('expire_index', '>', $block->block_index)->orderBy('expire_index', 'asc')->get()->sortByDesc('trading_price_normalized') : collect([]);
        $sell_orders = $token ? $token->giveOrders()->where('status', '=', 'open')->where('expire_index', '>', $block->block_index)->orderBy('expire_index', 'asc')->get()->sortBy('trading_price_normalized') : collect([]);
