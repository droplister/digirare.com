...
class CardsController extends Controller
{
    ...
    {
        $cards = Card::with('lifetimeMetrics', 'lastThirtyMetrics')->withCount('balances')
            ->orderBy('balances_count', 'desc')->paginate(100);
    ...
    public function show(Request $request, Card $card)
    {
        ...
        $buy_orders = $token ? $token->getOrders()->where('status', '=', 'open')->orderBy('expire_index', 'asc')->get() : collect([]);
        $sell_orders = $token ? $token->giveOrders()->where('status', '=', 'open')->orderBy('expire_index', 'asc')->get() : collect([]);
